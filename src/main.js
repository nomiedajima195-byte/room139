...
const fragmentShader = `
  ...
  void main() {
    ...
    float t = uTime * 0.12;
    vec3 c1 = vec3(0.88, 0.93, 1.0);
    vec3 c2 = vec3(1.0, 0.88, 0.94);
    vec3 c3 = vec3(0.94, 0.88, 1.0);
    float n1 = sin(uv.x * 2.0 + t) * 0.5 + 0.5;
    float n2 = cos(uv.y * 1.5 - t * 0.4) * 0.5 + 0.5;
...
app.addEventListener('click', (e) => {
  if (e.target !== app && e.target !== canvas) return;
...
function createNode(text, x, y) {
  ...
  let start = Date.now();
  const rx = Math.random() * 0.6 + 0.4;
  const ry = Math.random() * 0.6 + 0.4;
